...
public class Ocean {
	...
	boolean isValid(Cell c) {
		return ocean.isValid(c);
	}

	boolean isWall(Cell c) {
		return ocean.isWall(c);
	}

	boolean isShark(Cell c) {
		return ocean.isShark(c);
	}

	boolean isNemo(Cell c) {
		return ocean.isNemo(c);
	}

	boolean isMarlin(Cell c) {
		return ocean.isMarlin(c);
	}

	boolean isMarked(Cell c) {
		return ocean.isMarked(c);
	}

	int getMark(Cell c) {
		return ocean.getMark(c);
	}
	...
	// cria uma janela para exibição do labirinto dentro (o inteiro n
	// corresponde ao índice do labirinto)
	Ocean(int n) {
...
class OceanCanvas extends Canvas {
	...
	OceanCanvas(int n) {
		...
		f.setBounds(100, 100, cellSize * matrix.length + 8, cellSize * matrix.length + 26);
		f.add(this);
		...
		for (int x = 0; x < matrix.length; x++)
			for (int y = 0; y < matrix.length; y++) {
				...
				if (isWall(c)) {
					...
					g.fillOval(x * cellSize + cellSize / 4, y * cellSize + cellSize / 4, cellSize - cellSize / 2,
							cellSize - cellSize / 2);
				} else {
